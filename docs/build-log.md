...
## v1.2 (2026-09-14): Rundown navigator with in-tree CRUD
- core `index/outgoing.ts` (`collectRefs`, shared by the reverse index and the navigator), `index/rundownTree.ts` (`buildRundownTree`: Rundown → tiers → expeditions → layers/objectives/dimensions → zones; plain IPC-safe DTO). Alias = `AliasOverride >= 0 ? AliasOverride : ZoneAliasStart + LocalIndex`; `LocalIndex` may be a string enum, normalised with `parseEnumValue`. Problems: per block from `project.diagnostics`; zone problems = diagnostics under `Zones[i]`; expedition = its Rundown-field diagnostics + each linked block once; rundown = own + union of all linked blocks (no double counting of shared layouts). Disabled expeditions show 0 problems because the validator gates them off.
- `project/expeditionOps.ts`: add/duplicate/move/delete expedition, add/duplicate/delete zone (`nextLocalIndex`, dependents check). Duplicates use `insertRaw` so comments travel; LocalIndex bump keeps the string/int representation via `{ $enum }`.
- IPC `rundown:tree` (cached in the host with the reference index), `rundown:op`. Renderer: `RundownView` (third sidebar tab, default when a Rundown block exists), `LinkRow` pick/new/unlink on every reference (uses `RefPicker`; "new" opens `NewBlockDialog` with a `then` continuation that writes the new id into the field), header chip "in A1 Compromise · main layout" (`rundownLookup.placeOfBlock`), `goToZone` focuses `/Zones/i` and the list item flashes.
- Hook: `GTFO_NAV_CODE=tab:rundown[:<prefix>]`.
- Vanilla-only links (e.g. vanilla WardenObjective) are shown with a badge and are not expandable (no full vanilla data for LevelLayout/WardenObjective/Text).

## v1.3 / 0.3.1 (2026-09-14): generic plugin form, fold all, updater
- `renderer/src/editor/GenericJsonEditor.tsx`: schema-less form for `Custom/` (and meta/unknown) files. Edits go through `edit:apply` with a file-root target (no blockId), so span-exact formatting preservation applies. `REF_KEYS` maps well-known plugin keys (MainLevelLayout, LevelLayoutID, WaveSettings, …) to datablock types and renders them with `RefPicker`. Lists add a template shaped like the last item with scalars reset.
- Source view: `foldAll(keepRoot)` / `unfoldAll` on the editor handle (CodeMirror `foldAll`/`foldable`/`unfoldEffect`); buttons live in the pane header when the source is showing.
- Updater (`main/updater.ts`): `update:check` hits the GitHub releases API (10 s timeout, silent on failure) 2.5 s after start; `update:install` downloads the portable asset into `PORTABLE_EXECUTABLE_DIR` (the folder the user launched from — electron-builder portable runs from a temp unpack dir, so `process.execPath` is wrong for this), launches it and quits. In dev / non-portable builds it opens the release page instead. Progress via `update:progress`.
- Hot reload of the game itself was investigated and parked: nothing outside the game can rebuild the rundown screen; MTFO's own experimental "Reload Game Data" is considered unreliable by the friend. PartialData LiveEdit already picks up every save for the next level load.

## 0.3.2 (2026-09-13): trailing commas
- Friend hit `[Error :MTFO pDataBlock] Error While Adding Block: System.Text.Json.JsonReaderException: The JSON object contains a trailing comma` on a block saved by the tool. Root cause: every parse in the tool used jsonc-parser with `allowTrailingComma: true`, so a trailing comma was invisible to the validator, to the edit engine's post-edit sanity check and to the Source view lint, while PartialData hands each block's raw text to `JsonDocument.Parse` with comments allowed but trailing commas not.
- Decision: keep parsing lenient (a transient trailing comma while typing must not block the Source view) and detect the commas separately. `text/trailingCommas.ts`: `findTrailingCommas(text, tree)` scans the gap between a container's last child and its closing bracket with jsonc-parser's scanner (comments skipped); `stripTrailingCommas`.
- Validator rule **P003** (error) per comma, attributed to the enclosing block, with a fix that applies the new file-level EditOp `stripTrailingCommas` (one undo step). Source view marks them immediately from its local parse; the committed text then carries the validator's P003 too.
- Edit engine safety net in `EditSession.preview`: trailing commas present after an edit that do not map to one present before (offsets mapped through the applied edits) are removed from the result, so no form/tree/clipboard operation can leave one behind, while pre-existing ones stay (reported, not silently rewritten).
- A sampled corpus fuzz (remove first/mid/last, insert, insertRaw, setProperty, set-missing-leaf on ~60 containers per file: 37,575 ops over 259 files) produced no trailing comma and no parse error, so the op builders were not the leak; the likely route was the Source view, where a typed trailing comma passed the lenient parse unflagged and was saved. The corpus e2e test now asserts 0 P003.
